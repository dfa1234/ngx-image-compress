...
import {DataUrl, NgxImageCaptureModule, NgxImageCompressService} from 'ngx-image-compress';
...
describe('AppComponent', () => {
    ...
    it('should render in template Upload/Compress button', () => {
        ...
        expect(nativeCompiledElement.querySelector('button').textContent).toContain('Upload and compress Image');
    });
    ...
    it('should compress file', async () => {
        ...
        (mockNgxImageCompressService.compressFile as jasmine.Spy).and.returnValue(Promise.resolve(testDataUrl.substring(0, 50)));
        await component.compressFile();
        expect(mockNgxImageCompressService.uploadFile).toHaveBeenCalled();
        expect(component.imgResultBeforeCompress).toEqual(testDataUrl.substring(0, 100));
        expect(mockNgxImageCompressService.compressFile).toHaveBeenCalled();
        expect(component.imgResultAfterCompress).toEqual(testDataUrl.substring(0, 50));
    });
    ...
    it('should upload multiple files', async () => {
        (mockNgxImageCompressService.uploadMultipleFiles as jasmine.Spy).and.returnValue(
            Promise.resolve([
                {
    ...
    it('should upload, compress and resize at a maximum width and height', async () => {
        ...
        expect(component.imgResultAfterResize).toEqual(testDataUrl.substring(0, 50));
    });
    ...
    it('should upload, compress and resize at a 1 megabytes max', async () => {
        (mockNgxImageCompressService.uploadAndGetImageWithMaxSize as jasmine.Spy).and.returnValue(
            Promise.resolve(testDataUrl.substring(0, 150))
        );
    ...
    it('should upload, compress and be unable to resize', async () => {
        (mockNgxImageCompressService.uploadAndGetImageWithMaxSize as jasmine.Spy).and.returnValue(
            Promise.reject(testDataUrl.substring(0, 150))
        );
        await component.uploadAndReturnWithMaxSize();
        expect(mockNgxImageCompressService.uploadAndGetImageWithMaxSize).toHaveBeenCalled();
    });
